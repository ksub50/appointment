...
public class Appointment implements EntryPoint {
	public void onModuleLoad( ) {
        ...
        daTextbox.addBlurHandler(new BlurHandler() {
			...
			Double totalsalaryDoubleValue = basicDoubleValue+daDoubleValue+hraDoubleValue;
			String totalsalaryValue = String.valueOf(totalsalaryDoubleValue);
		...
		timeLabel.setText("Enter Appointment Time ");
		final TextBox timeTextbox = new TextBox();
		
		Button answerButton = new Button();
		answerButton.setText("Save");
		final Label replyLabel = new Label();

		ClickHandler handler = new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				String value = dateTextbox.getText();
				if (value.isEmpty()) {
					replyLabel.setText("Enter a valid date.");
				} else {
					try {
						DateTimeFormat dateParser = DateTimeFormat.getFormat(PredefinedFormat.DATE_SHORT);
						Date d = dateParser.parse(value);
						replyLabel.setText("You entered " + d.toString());
					} catch(Exception e) {
						replyLabel.setText("Error parsing date " + e.toString());
					}
				}
			}
		};
		answerButton.addClickHandler(handler);
